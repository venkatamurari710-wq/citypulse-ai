...
export default function HowItWorksSection() {
  return (
    <section id="how-it-works" className="py-20 lg:py-28 bg-slate-50/60 relative overflow-hidden transition-colors duration-300">
          ...
          {steps.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                className="relative bg-white rounded-3xl p-7 border border-slate-200/80 shadow-card hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between group"
              >
                <div>
                  {/* Step Header */}
                  <div className="flex items-center justify-between mb-6">
                    <span className="font-grotesk font-extrabold text-3xl text-slate-300 group-hover:text-[#1D4ED8] transition-colors">
                      {item.step}
                    </span>
                    <div
                      className={`w-12 h-12 rounded-2xl bg-gradient-to-tr ${item.color} text-white flex items-center justify-center shadow-md shadow-blue-500/15 group-hover:scale-110 transition-transform`}
                    >
                      <Icon className="w-6 h-6" />
                    </div>
                  </div>

                  <h3 className="text-xl font-grotesk font-bold text-[#0F2C59] mb-2">{item.title}</h3>
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                  <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 group-hover:bg-blue-50 group-hover:text-[#1D4ED8] transition-colors">
                    {item.highlight}
                  </span>
                  {idx < steps.length - 1 && (
                    <ArrowRight className="w-4 h-4 text-slate-400 group-hover:text-[#1D4ED8] group-hover:translate-x-1 transition-all hidden lg:block" />
                  )}
                </div>
              </motion.div>
            );
