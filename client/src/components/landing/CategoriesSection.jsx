...
export default function CategoriesSection() {
  return (
    <section className="py-20 bg-white relative transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-14 font-sans">
          ...
          <h2 className="text-3xl sm:text-4xl font-grotesk font-extrabold text-[#0F2C59] mb-3">
            What Can You Report?
          </h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 font-sans">
          {categories.map((cat, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="flex items-center gap-3 bg-slate-50/80 border border-slate-200/90 rounded-2xl px-4 py-3.5 hover:border-[#1D4ED8]/40 hover:bg-white transition-all duration-200 cursor-default shadow-xs group hover:-translate-y-0.5"
            >
              <span className="text-2xl group-hover:scale-125 transition-transform duration-200">
                {cat.emoji}
              </span>
              <div className="flex flex-col min-w-0">
                <span className="text-sm font-bold text-[#0F2C59] truncate group-hover:text-[#1D4ED8] transition-colors">
                  {cat.name}
                </span>
